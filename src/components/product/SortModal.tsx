...
export const SortModal = ({
  ...
  const handleSelect = (opt: SortType) => {
    onSelect(opt);
    onClose();
  };
      ...
      <Pressable style={styles.modalOverlay} onPress={onClose}>
        ...
        >
          {SORT_OPTIONS.map(opt => (
            ...
            >
              <Text style={[styles.sortOptionText, selected === opt && styles.sortOptionTextActive]}>
                {opt}
              </Text>
...
const styles = StyleSheet.create({
  ...
  sortOptionText: { fontSize: 14, color: colors.black, fontWeight: '400' },
  sortOptionTextActive: { color: colors.primary, fontWeight: '600' },
  sortCheckMark: { fontSize: 14, color: colors.primary, fontWeight: '700' },
});
