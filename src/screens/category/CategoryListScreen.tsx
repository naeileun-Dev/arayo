...
export const CategoryListScreen = () => {
  ...
  const activeFilterCount = useMemo(() => countActiveFilters(appliedFilter), [appliedFilter]);
  ...
  const handleCategoryToggle = useCallback((cat: string, sub: string) => {
          ...
          >
            <View style={styles.iconMagazine}>
              {[0, 1, 2].map(i => (
                <View
                  key={i}
                  style={[styles.iconMagazineLine, viewType === 'magazine' && { backgroundColor: C.primary }]}
                />
              ))}
            </View>
          </TouchableOpacity>
          ...
          >
            <View style={styles.iconGrid}>
              {[0, 1, 2, 3].map(i => (
                <View
                  key={i}
                  style={[styles.iconGridCell, viewType === 'grid' && { backgroundColor: C.primary }]}
                />
              ))}
            </View>
          </TouchableOpacity>
  ...
  const contentContainerStyle = useMemo(() => {
    if (compareVisible) return [styles.listContent, { paddingBottom: SCREEN_HEIGHT * 0.65 }];
    if (compareList.length > 0) return [styles.listContent, { paddingBottom: 80 }];
    return styles.listContent;
  }, [compareVisible, compareList.length]);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {selectedCategories.length > 0 ? selectedCategories[0].category : '전체'}
        </Text>
        <TouchableOpacity
          style={styles.headerIconBtn}
...
const styles = StyleSheet.create({
  ...
  viewToggleBtnActive: {
    borderColor: C.primary,
    backgroundColor: C.viewToggleBg,
  },
  iconMagazine: {
    gap: 3,
  },
